...
void lock_pairs(void)
{
    for (int i = 0; i < pair_count; i++)
    {
        ...
        if (!will_create_cycle(winner, loser))
        {
...
bool will_create_cycle(int winner, int loser)
{
    // If loser is a direct winner, adding winner creates cycle
    if (locked[loser][winner])
    {
        return true;
    }

    // Check if winner has any direct or indirect losers
    for (int i = 0; i < candidate_count; i++)
    {
        if (locked[i][winner])
        {
            if (will_create_cycle(i, loser))
            {
                return true;
            }
        }
